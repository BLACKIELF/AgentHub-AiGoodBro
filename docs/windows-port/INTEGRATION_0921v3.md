# Windows 交接与源码集成 · 0921v5

相较 0921v4，本版增加按目录手动查询额度、旧结果标识和迟到结果保护；不引入登录或切号。
...
- 每个已关联目录有“读取额度”按钮，无需改当前查看来源，尚无 sessions 的目录也可查询。
- 复用已有 Codex app-server 只读接口并传入该目录；成功回执只有目录 ID、读取时间和已返回的窗口，不带邮箱、路径或原始响应。
- 未返回的窗口不展示；没有任何有效窗口、非法百分比或调用失败均返回不可用，不替换为 0。真实 0% 正常显示。
- 界面只在点击时读取，按钮防连点；后端最多 2 个目录查询同时在途，超限报错，不排队扫全部账号。自动清理只更新时间标签，不发请求。
- 单行记录只在当前界面内存里保留；刷新失败、超过 5 分钟或达到已知重置时间时显示“上次记录，需重读”，不推断额度已恢复。关闭后不持久化额度。
- 后端返回前重查 ID 与目录关联；前端校验 ID、数值和窗口，并在卸载时作废旧请求。移除关联不会被迟到结果恢复。
- 注意：查询成功仅证明该目录启动的 app-server 返回额度；尚未实现受管身份验证，不可作为自动切号/暖号授权。相同目录内外部换号后的旧值也只能视作历史读取记录。

新增文件：Tauri commands/profile_quota.rs、Web ProfileQuota.tsx；
既有文件仅增加命令注册、并发名额、行入口、测试和说明。Mac 文件不变。
