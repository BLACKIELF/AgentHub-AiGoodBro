...
/// Use the reserved built-in provider, which recent CLIs reject redefining.
/// These scalar overrides affect this child only; no config file is edited.
pub fn official_arguments() -> Vec<String> {
    ...
    .into_iter()
    .flat_map(|value| ["--config".to_owned(), value.to_owned()])
    .collect()
}

/// Check the CLI's effective config before requesting any account evidence.
pub fn validate_official_config(response: &Value) -> anyhow::Result<()> {
    ...
    for (key, expected) in [
        ...
        anyhow::ensure!(
            config.get(key).and_then(Value::as_str) == Some(expected),
    ...
    if let Some(providers) = providers {
        let map = providers
            .as_object()
            .ok_or_else(|| anyhow::anyhow!("Invalid CLI provider catalog"))?;
        anyhow::ensure!(
            !map.contains_key("openai"),
            "CLI built-in provider must not be redefined"
        );
    }
    Ok(())
}
...
impl WorkflowPreferences {
    pub fn validate(&self) -> anyhow::Result<()> {
        ...
        for (id, value) in &self.profiles {
            ...
            anyhow::ensure!(
                value.model.as_deref().is_none_or(valid_model),
                "Invalid model"
            );
            anyhow::ensure!(
                value.effort.as_deref().is_none_or(valid_effort),
                "Invalid effort"
            );
            anyhow::ensure!(
                value.model.is_some() || value.effort.is_none(),
...
pub fn parse_models(value: &Value) -> anyhow::Result<Vec<WorkflowModel>> {
    ...
    for row in rows {
        ...
        let Some(default_effort) = row
            .get("defaultReasoningEffort")
            .and_then(Value::as_str)
            .filter(|v| valid_effort(v))
        else {
            continue;
        };
        let Some(raw_efforts) = row
        ...
        let label = row
            ...
            .to_owned();
        anyhow::ensure!(
            !models.iter().any(|model: &WorkflowModel| model.id == id),
            "Duplicate model"
        );
...
pub fn resolve_selection(
    ...
    anyhow::ensure!(
        model.efforts.iter().any(|value| value == effort),
        "Selected effort unavailable"
    );
...
pub fn validate_startup(
    ...
    let limits = if let Some(buckets) = rate_limits
        .get("rateLimitsByLimitId")
        .filter(|v| !v.is_null())
    {
        buckets.get("codex")
    } else {
        rate_limits.get("rateLimits")
    }
    .ok_or_else(|| anyhow::anyhow!("Official quota unavailable"))?;
    let credits = limits
        .get("credits")
        .ok_or_else(|| anyhow::anyhow!("Credit state unknown"))?;
    anyhow::ensure!(
        credits.get("hasCredits").and_then(Value::as_bool) == Some(false)
            && credits.get("unlimited").and_then(Value::as_bool) == Some(false),
        "Paid or unlimited credit fallback is not eligible"
    );
    ...
    anyhow::ensure!(
        balance == Some(0.0),
        "Credit balance must be explicitly zero"
    );
    let mut durations = std::collections::HashSet::new();
    for name in ["primary", "secondary"] {
        let Some(window) = limits.get(name).filter(|v| !v.is_null()) else {
            continue;
        };
        ...
        anyhow::ensure!(
            used.is_some_and(|v| v.is_finite() && (0.0..100.0).contains(&v))
                && reset.is_some_and(|v| v > now_seconds)
                && duration.is_some_and(
                    |v| matches!(v, 300 | 10_080) || (28 * 24 * 60..=31 * 24 * 60).contains(&v)
                ),
            "Quota exhausted, stale or unknown"
        );
        let duration = duration.unwrap();
        let kind = if duration >= 28 * 24 * 60 {
            "monthly"
    ...
    anyhow::ensure!(
        durations.contains("five_hour")
            || (plan == Some("prolite") && durations.contains("seven_day")),
        "Required current quota window unavailable"
    );
    Ok(())
}
...
pub fn interactive_arguments(model: &str, effort: &str) -> anyhow::Result<Vec<String>> {
    anyhow::ensure!(
        valid_model(model) && valid_effort(effort),
...
mod tests {
    ...
    fn participation_can_be_disabled_without_any_cli_or_quota_state() {
        ...
        store
            .set_selection(1, Some("synthetic-model".into()), Some("high".into()))
            .unwrap();
        store.set_participating(1, false).unwrap();
        let round_trip: WorkflowPreferences =
        ...
        assert!(store
            .set_selection(1, Some("bad; command".into()), Some("high".into()))
            .is_err());
    }
    #[test]
    fn only_advertised_model_effort_pairs_can_start() {
        ...
        assert_eq!(
            resolve_selection(&WorkflowPreference::default(), &models).unwrap(),
            ("synthetic-model".into(), "high".into())
        );
        let invalid = WorkflowPreference {
        ...
        assert!(!interactive_arguments("synthetic-model", "high")
            .unwrap()
            .iter()
            .any(|arg| arg == "exec" || arg == "--dangerously-bypass-approvals-and-sandbox"));
    }
    #[test]
    fn startup_requires_official_identity_future_quota_and_known_zero_credit() {
        ...
        for (key, value) in [
            ("hasCredits", json!(true)),
            ("unlimited", json!(true)),
            ("balance", json!(1)),
            ("balance", Value::Null),
        ] {
            let mut altered = limits.clone();
            altered["rateLimits"]["credits"][key] = value;
        ...
        let mut exhausted = limits.clone();
        exhausted["rateLimits"]["primary"]["usedPercent"] = json!(100);
        assert!(validate_startup(&account, &exhausted, 100).is_err());
        let mut malformed_map = limits.clone();
        malformed_map["rateLimitsByLimitId"] = json!({"other":{}});
        assert!(validate_startup(&account, &malformed_map, 100).is_err());
        let mut weekly = limits.clone();
        weekly["rateLimits"]["primary"]["windowDurationMins"] = json!(10080);
        ...
        assert!(validate_startup(&account, &weekly, 100).is_err());
        let mut duplicate = limits.clone();
        duplicate["rateLimits"]["secondary"] = duplicate["rateLimits"]["primary"].clone();
        assert!(validate_startup(&account, &duplicate, 100).is_err());
        let mut unknown = limits.clone();
        unknown["rateLimits"]["primary"]["windowDurationMins"] = json!(600);
        assert!(validate_startup(&account, &unknown, 100).is_err());
    }
    #[test]
    fn official_route_uses_builtin_provider_and_removes_external_auth() {
        let args = official_arguments();
        assert!(args.contains(&"model_provider=\"openai\"".to_owned()));
        let model_endpoint = "openai_base_url=\"https://chatgpt.com/backend-api/codex\"";
        assert!(args.contains(&model_endpoint.to_owned()));
        assert!(!args.iter().any(|arg| arg.starts_with("model_providers")));
    ...
    #[test]
    fn effective_config_rejects_missing_or_redirected_subscription_settings() {
        let config = json!({"config":{
            ...
            "forced_login_method":"chatgpt",
            "cli_auth_credentials_store":"file",
            "model_providers":{"synthetic-custom":{"base_url":"https://example.invalid"}}
        }});
        ...
        for key in [
            ...
            changed["config"][key] = json!("synthetic-wrong-value");
            assert!(validate_official_config(&changed).is_err());
            changed["config"][key] = Value::Null;
            assert!(validate_official_config(&changed).is_err());
        }
        let mut redefined = config.clone();
        redefined["config"]["model_providers"]["openai"] = json!({"env_key":"FAKE_KEY"});
        assert!(validate_official_config(&redefined).is_err());
        redefined["config"]["model_providers"] = json!([]);
        assert!(validate_official_config(&redefined).is_err());
        assert!(validate_official_config(&Value::Null).is_err());
    }
}
